...
/**
 * I use a new version of code from ChatGPT therefore I comment the old code from line 305 to 610
 * New version of code is form line 611
 */
...
// // 工具函数：校验用户组&状态
// function validGroup(g){ return ["admin","staff"].includes(g); }
// function validStatus(s){ return ["active","inactive"].includes(s); }
...
//   try {
//     // ====== 调试日志：确认命中的是本函数/本文件 ======
//     console.log("[LOGIN] handler file:", __filename);
...
//     console.log('[DB] using schema:', db.db);
//     // 新增 ↑↑↑
...
//          LIMIT 1
//       `;
//       // ====== 调试日志：打印实际执行的 SQL 与参数 ======
//       console.log("[SQL]", sql.trim(), " -- params:", [login, login]);

//       const [[u]] = await conn.query(sql, [login, login]);

//       // ====== 调试日志：返回列名，用于确认是否为 user_password ======
//       console.log("[SQL] result keys:", u ? Object.keys(u) : u);
...
//       const ok = await bcrypt.compare(password, u.user_password);
//       // ====== 调试日志：密码校验结果 ======
//       console.log("[LOGIN] password compare:", ok);
...
//     }
//   } catch (err) {
//     console.error("[LOGIN] error:", err); // ====== 调试日志：打印错误 ======
//     res.status(500).json({ ok: false, error: err.message });
//   }
// }
...
// ===== 调试日志：用环境变量 DEBUG_AUTH 控制（1/true/yes/on） =====
...
function validGroup(g) {
  return ["admin", "staff"].includes(g);
}
function validStatus(s) {
  return ["active", "inactive"].includes(s);
}
...
async function createUser(req, res) {
  ...
  try {
    ...
    const conn = await getPool("orders").getConnection(); // users 表在 orders 对应的库
    try {
      const [result] = await conn.query(
        "INSERT INTO users (user_name, user_password, user_group, real_name, email, status) VALUES (?, ?, ?, ?, ?, ?)",
        [user_name, hash, user_group, real_name, email, status]
...
/**
 * POST /api/login
 * body: { login, password }  // login = user_name 或 email
 */
async function login(req, res) {
  const { login, password } = req.body || {};
  if (!login || !password) {
  ...
  try {
    d("[LOGIN] handler file:", __filename);
    d("[LOGIN] payload:", { login });
    ...
    try {
      ...
      const [[u]] = await conn.query(sql, [login, login]);
      d("[SQL] result keys:", u ? Object.keys(u) : u);
...
async function updateStatus(req, res) {
  ...
  try {
    const conn = await getPool("orders").getConnection();
    try {
      const [r] = await conn.query("UPDATE users SET status=? WHERE user_id=?", [
        status,
        userId,
      ]);
      res.json({ ok: true, affectedRows: r.affectedRows });
